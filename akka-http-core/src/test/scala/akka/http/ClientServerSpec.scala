...
class ClientServerSpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  "The server-side HTTP infrastructure" should {
    ...
    "report failure if bind fails" in {
      ...
      val Http.ServerBinding(localAddress1) = Await.result(mm1.get(key), 3.seconds)
      localAddress1.getHostName shouldEqual hostname
      localAddress1.getPort shouldEqual port
      ...
      val failure = intercept[akka.stream.io.StreamTcp.IncomingTcpException] {
        val Http.ServerBinding(localAddress2) = Await.result(mm2.get(key), 3.seconds)
      }
