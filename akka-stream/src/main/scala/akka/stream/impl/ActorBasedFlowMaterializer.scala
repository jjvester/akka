...
case class ActorBasedFlowMaterializer(override val settings: MaterializerSettings,
  ...
  @tailrec private[this] def processorChain(topProcessor: Processor[_, _],
                                            ...
                                            flowName: String,
                                            n: Int,
                                            materailizedMap: MaterializedMap): (Processor[_, _], MaterializedMap) =
    ops match {
      case op :: tail ⇒
        ...
        processorChain(opProcessor, tail, flowName, n - 1, materailizedMap.merge(opMap))
      case Nil ⇒
        (topProcessor, materailizedMap)
    }
  ...
  override def materialize[In, Out](source: Source[In], sink: Sink[Out], rawOps: List[Ast.AstNode], keys: List[Key]): MaterializedMap = {
    ...
    val materializedMap = if (keys.isEmpty) sourceSinkMap
    else {
      (sourceSinkMap /: keys) {
